...
class StateVector:
    ...
    def apply_unitary_operation(self, operation):
        ...
        # nnz = np.count_nonzero(operation)
        # print(nnz)
        # print(f"{nnz}/{operation.size} = {nnz/operation.size*100}% sparseness")

        # print(timeit(lambda: operation @ self.state_vector, number=1))
...
class CircuitUnitaryOperation:
    ...
    def get_combined_operation_for_qubit(operation, q, N):
        ...
        for i in range(0, N):
            ...
            print(f"Qubit: {i+1}", f"{round(t2 - t1, 6)*1000} ms")
    ...
    def get_combined_operation_for_cnot(control, target, N):
        ...
        print(timeit(lambda: combined_operation_zero + combined_operation_one, number=2))

        a = combined_operation_zero + combined_operation_one
        return a
